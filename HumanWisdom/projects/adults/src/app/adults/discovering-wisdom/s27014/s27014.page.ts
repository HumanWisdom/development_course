import { Location } from '@angular/common';
import { Component, OnInit } from '@angular/core';
import { Router } from '@angular/router';
import { AdultsService } from "../../adults.service";

@Component({
  selector: 'app-s27014',
  templateUrl: './s27014.page.html',
  styleUrls: ['./s27014.page.scss'],
})
export class S27014Page implements OnInit {

  bg_tn = "bg_purple_blue"
  bg_cft = "bg_purple_blue"
  bg = "purple_blue_w1"
  hint = "Some people think it comes with age or experience, but what we are referring to is to live with this wisdom even when you are young."
  toc = "discovering-wisdom/s27001"
  path = this.router.url

  userId: any
  saveUsername = JSON.parse(localStorage.getItem("saveUsername"))
  qrList = JSON.parse(localStorage.getItem("qrList"))
  moduleId = localStorage.getItem("moduleId")
  screenType = localStorage.getItem("reflection")
  screenNumber = 27014
  startTime: any
  endTime: any
  totalTime: any
  bookmark: any
  rId = 792
  reflection: any
  reflectionA: any
  //r27014 = JSON.parse(sessionStorage.getItem("r27014"))
  r27014 = sessionStorage.getItem("r27014") !== 'null' ? sessionStorage.getItem("r27014") : ''
  shared: any
  confirmed: any


  constructor(private router: Router,
    private service: AdultsService,
    private location: Location) { }

  ngOnInit() {
    this.createScreen()


    this.reflectionA = this.qrList.ListOfReflection


    this.findReflection()
    if (this.saveUsername == false) { this.userId = JSON.parse(sessionStorage.getItem("userId")) }
    else { this.userId = JSON.parse(localStorage.getItem("userId")) }
    this.startTime = Date.now();


  }
  sharedForum(e) {
    console.log(e)
    this.shared = e
  }

  confirmShare() {
    this.confirmed = true
  }
  createScreen() {
    this.service.createScreen({
      "ScrId": 0,
      "ModuleId": this.moduleId,
      "GSetID": this.screenType,
      "ScreenNo": this.screenNumber
    }).subscribe(res => {

    })


  }

  findReflection() {
    for (var i = 0; i < this.reflectionA.length; i++) {



      if (this.rId == this.reflectionA[i].ReflectionId) {
        this.reflection = this.reflectionA[i].Que
        // this.optionList.push(this.questionA[i])
      }

    }
    console.log(this.reflection)

  }


  submitProgress(e) {
    console.log("returned response", e)
    this.endTime = Date.now();
    this.totalTime = this.endTime - this.startTime;
    sessionStorage.setItem("r27014", JSON.stringify(e))
    //this.r27014 = sessionStorage.getItem("r27014")
    console.log(this.r27014)
    if (this.r27014 != "undefined") {

      this.service.submitProgressReflection({
        "ScrNumber": this.screenNumber,
        "UserId": this.userId,
        "BookMark": this.bookmark,
        "ModuleId": this.moduleId,
        "screenType": this.screenType,
        "timeSpent": this.totalTime,
        "ReflectionId": this.rId,
        "Resp": this.r27014
      }).subscribe(res => {

      },
        error => {
          console.log(error)
          this.router.navigate(['/adults/discovering-wisdom/s27015'])

        },
        () => {
          this.router.navigate(['/adults/discovering-wisdom/s27015'])
        })
    }

    else {
      this.router.navigate(['/adults/discovering-wisdom/s27015'])

    }




  }

  previous() {
    localStorage.setItem("pageaction", 'prev')
    this.router.navigate(['/adults/discovering-wisdom/s27013'])
  }

  ngOnDestroy() {



  }


}
