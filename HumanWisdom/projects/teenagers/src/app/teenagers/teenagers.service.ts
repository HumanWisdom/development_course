...
export class TeenagersService {
  ...
  ensureModuleContextForUrl(url: string): void {
    ...
    const cachedMapStr = sessionStorage.getItem('urlModuleMap_teen');
    if (cachedMapStr) {
      try {
    ...
    this.getModuleList().subscribe(res => {
        if (res && res.length) {
          ...
          sessionStorage.setItem('urlModuleMap_teen', JSON.stringify(this.urlModuleMap));
          this.checkAndActivateModule(url, segments, this.urlModuleMap);
        }
    });
  }
  ...
  private setModuleState(id: any, res: any): void {
    ...
    if(Number.isNaN(mediaPercent)){
        localStorage.setItem("mediaPercent", "0")
    }else{
        localStorage.setItem("mediaPercent", mediaPercent.toString())
    }
