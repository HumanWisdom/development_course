...
export class S157003Page implements OnInit {
  ...
  onSwipe($event) {
      if (this.lastClick >= (Date.now() - this.delay))
    {
      ...
      if(eventText.includes("right")){
        ...
        if (this.currentDay == 6 && this.slideStart == this.totalSlidesCount) return;
  ...
  next() {
    if (this.currentDay == 6 && this.slideStart == this.totalSlidesCount) {
      return;
    }
    window.scrollTo(0,0);
    this.nextDay = null;
  ...
  try {
    ...
    if (activeSlides && activeSlides.length > 0) {
      ...
      if (journalWe != null && journalWe.dataset && journalWe.dataset.hint) {
        ...
        this.hintMessage = this.hintValue.hint;
