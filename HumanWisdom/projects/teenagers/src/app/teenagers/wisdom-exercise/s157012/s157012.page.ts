...
export class S157012Page implements OnInit {
  ...
  constructor(private elementRef: ElementRef,
    public service: TeenagersService, private adult: TeenagersService,public router:Router, private location: Location) {
  ...
  next() {
    ...
    setTimeout(() => {
      if (this.slideStart < this.totalSlidesCount) {
        ...
        this.vistedScreens.push({
          "ScreenNo": '157012p' + (parseInt(this.screenNumber.substring(7, this.screenNumber.length))),
          "ModuleID": 157,
          "SessionID": 0,
        })
        if(this.currentDay>this.totaldays){
  ...
  onSwipe($event) {
    ...
    if (eventText.includes("right")) {
      ...
      if (this.currentDay == 9 && this.slideStart == this.totalSlidesCount) {
        // Guard for last slide of day 9
      } else {
