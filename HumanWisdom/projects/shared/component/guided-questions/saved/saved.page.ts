...
import { ProgramModel, ProgramType } from '../../../models/program-model';
@Component({
  selector: 'app-saved',
...
export class savedPage implements OnInit {
  ...
  ngOnInit() {
    if (SharedService.ProgramId == ProgramType.Adults) {
      this.isAdults = true;
      this.programType = ProgramType.Adults;
    } else {
      this.isAdults = false;
    }
    this.userId = JSON.parse(localStorage.getItem("userId"));

    this.id = localStorage.getItem("topicId");
    this.GetGuidedQs_Topics();
  }

  GetGuidedQs_Topics() {
    this.service.GetGuidedQs_TopicsId(this.id).subscribe(res => {
      if (res) {
        this.moduleIds = res[0]['moduleIds'].split(',');
        console.log(res);
  ...
  goBack() {
   ...
   GetModuleDataBasedOnProgramType() {
      this.service.getModules(this.programType).subscribe(res => {
        res = res.filter((d) => this.moduleIds.find((e) => e === d['moduleId'].toString()));
        this.moduleData = res;
      });
    }
